// Crée des fiches "squelettes" fiables à partir de la liste 200 (InfoFlora) :
// identité (nom latin) + classification (famille) + planche botanique Thomé
// (domaine public) récupérée sur Wikimedia Commons.
//
...
//   export $(grep -v '^#' .env | xargs) && node scripts/scaffold-plantes.mjs   # tout, vers Turso
...
// --- liste d'espèces ---
let especes = JSON.parse(readFileSync('src/data/infoflora-200.json', 'utf8'));
...
async function findIllustration(latin) {
  ...
  const base = `Illustration_${latin.replace(/ /g, '_')}`;
...
const client = dry ? null : createClient({ url: dbUrl, authToken: process.env.TURSO_AUTH_TOKEN || undefined });
...
for (const { latin_name, family } of especes) {
  ...
  if (!dry) {
    const data = { classification: { famille: [family] } };
    if (image_ref) {
